'use strict';
/* WhatsApp Connector — QR + Pairing Code. Deploy on Railway/Render/Fly. */
    ...
    sock.ev.on('connection.update', async (u) => {
      ...
      if (connection === 'close') {
        ...
        if (code === DisconnectReason.loggedOut) {
          ...
          reconnectTimer = setTimeout(() => { startSocket(mode).catch(() => {}); }, 3000);
        } else {
...
app.post('/pair', async (req, res) => {
  ...
  try {
    // Force a fresh socket so pairing code mode is used
    if (sock) {
      try { sock.end(undefined); } catch (_) {}
      sock = null;
      await new Promise(r => setTimeout(r, 500));
    }
    startSocket('pair').catch(() => {});
    // Wait for socket to be ready
    for (let i = 0; i < 30; i++) {
      await new Promise(r => setTimeout(r, 300));
      if (sock) break;
    }
    if (!sock) return fail(res, 'Socket failed to start', 'CONNECTOR_ERROR', 500);

    // Small settle time before requesting
